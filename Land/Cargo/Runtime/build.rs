...
async fn main() -> StdResult<(), Box<dyn Error>> {
	...
	ctx.with(|ctx| {
		for dir_ent in WalkDir::new(BUNDLE_DIR).into_iter().flatten() {
			...
			let filename = dir_ent.path().with_extension("lrt").to_string_lossy().to_string();
...
fn compress_bytecode(bundles_dir: &str, source_files: Vec<String>) -> io::Result<usize> {
	...
	let dictionary_path = bundle_path.join("compression.dict").to_string_lossy().to_string();
	...
	cmd.args(["--train", "--train-fastcover=steps=40", "--maxdict=20K", "-o", &dictionary_path]);
	if file_count < 5 {
