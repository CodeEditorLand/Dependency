...
impl<'js> URL<'js> {
	...
	pub fn new(ctx: Ctx<'js>, input: Value<'js>, base: Opt<Value<'js>>) -> Result<Self> {
		if let Some(base) = base.0 {
			...
			let url = base.join(path_string.as_str()).or_throw_msg(&ctx, "Invalid URL")?;
	...
	pub fn to_string(&self) -> String {
		...
		let hash = &self.hash;
		let hash = if !hash.is_empty() { format!("#{}", &hash) } else { String::from("") };

		format!("{}://{}{}{}{}", &self.protocol, &self.host, &self.pathname, &search, &hash)
	}
...
fn split_colon<'js>(ctx: &Ctx, s: &'js str) -> Result<(&'js str, &'js str)> {
	...
	if parts.next().is_some() || (first.is_empty() && second.is_empty()) {
		return Err(Exception::throw_message(ctx, "String contains more than one ':'"));
	}
	Ok((first, second))
}
