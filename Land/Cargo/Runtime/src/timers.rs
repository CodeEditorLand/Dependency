...
fn get_current_time_millis() -> usize {
	SystemTime::now().duration_since(UNIX_EPOCH).map(|t| t.as_millis() as usize).unwrap_or(0)
}
...
fn set_timeout_interval<'js>(
	...
	timeouts.lock().unwrap().push(Timeout { cb: Some(cb.clone()), timeout, id, repeating, delay });
...
pub fn init(ctx: &Ctx<'_>) -> Result<()> {
	...
	globals
		.set("clearTimeout", Func::from(move |id: usize| clear_timeout_interval(&timeouts3, id)))?;
