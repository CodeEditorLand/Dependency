...
impl<'js> XMLParser<'js> {
	...
	fn process_end(
		...
		if current_obj.contains_key(tag)? {
			...
			current_obj.prop(tag, Property::from(value).configurable().enumerable().writable())?;
		}
		Ok(())
	}
			...
			for attribute in tag.attributes() {
				*has_attributes = true;
				...
				let key = if !self.attribute_name_prefix.is_empty() {
					...
					reader.decoder().decode(&key_bytes).or_throw(ctx)?.to_string()
				} else {
					reader.decoder().decode(key_slice).or_throw(ctx)?.to_string()
				};

				let mut value =
					reader.decoder().decode(attr.value.as_ref()).or_throw(ctx)?.to_string();
...
impl XmlText {
	...
	fn new(value: String) -> Self {
		XmlText { value: escape_element(&value) }
	}
...
impl<'js> XmlNode<'js> {
	...
	fn new(name: String, children: Opt<Vec<Value<'js>>>) -> Result<Self> {
		let node =
			XmlNode { name, attributes: Vec::new(), children: children.0.unwrap_or_default() };
