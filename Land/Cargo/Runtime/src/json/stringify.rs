...
fn write_primitive(context: &mut IterationContext, add_comma: bool) -> Result<bool> {
	...
	match type_of {
		...
		Type::Int => {
			let mut buffer = itoa::Buffer::new();
			context.result.push_str(buffer.format(value.as_int().unwrap()))
		}
		Type::Float => {
...
fn detect_circular_reference(
	...
	if ancestors.iter().any(|(ptr, _)| ptr == &current_ptr) {
		...
		let mut path = iter.rev().take(4).rev().fold(String::new(), |mut acc, (_, key)| {
			if !key.starts_with('[') {
				acc.push('.');
			}
			acc.push_str(key);
	...
	ancestors.push((
		current_ptr,
		key.map(|k| k.to_string()).unwrap_or_else(|| format!("[{}]", index.unwrap_or_default())),
	));
