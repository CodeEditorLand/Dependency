...
fn stringify_primitive<'js>(
	...
	match value_type {
		...
		Type::Bool => result.push_str(if value.as_bool().unwrap() { "true" } else { "false" }),
		Type::Int => {
		...
		Type::Float => {
			...
			result.push_str(match float_to_string(&mut buffer, value.as_float().unwrap()) {
				Ok(v) => v,
				Err(v) => v,
			})
		}
...
fn stringify_value<'js>(
	...
	stack.push(StringifyItem { value: Some(obj), depth: 0, key: None, end: None, expand: false });

	while let Some(StringifyItem { value, depth, key, end, expand }) = stack.pop() {
		if let Some(end) = end {
			if expand {
								...
								if let Some(class_name) = class_name {
									...
									is_typed_array = matches!(
										...
										"Int8Array"
											...
											| "Int64Array" | "Uint64Array" | "Float32Array"
											| "Float64Array" | "Buffer"
									)
								}
