...
impl Resolver for BinaryResolver {
	fn resolve(&mut self, _ctx: &Ctx, base: &str, name: &str) -> Result<String> {
		...
		let cache_path =
			if normalized_path.starts_with("./") { &normalized_path[2..] } else { normalized_path };
	...
	T: RawLoader + 'static,
{
	...
	unsafe fn raw_load<'js>(&mut self, ctx: &Ctx<'js>, name: &str) -> Result<Module<'js>> {
		...
		let name = if name.starts_with("./") { &name[2..] } else { name };
...
impl Vm {
	...
	pub async fn new() -> StdResult<Self, Box<dyn std::error::Error + Send + Sync>> {
		...
		let gc_threshold_mb: usize =
			env::var("LLRT_GC_THRESHOLD_MB").unwrap_or("20".into()).parse().unwrap_or(20);

		let runtime = AsyncRuntime::new()?;
		runtime.set_max_stack_size(512 * 1024).await;
		runtime.set_gc_threshold(gc_threshold_mb * 1024 * 1024).await; //20mb
	...
	{
		ctx.with(|ctx| {
			f(ctx.clone()).catch(&ctx).unwrap_or_else(|err| Self::print_error_and_exit(&ctx, err));
		})
		.await;
	}
	...
	pub fn print_error_and_exit<'js>(ctx: &Ctx<'js>, err: CaughtError<'js>) -> ! {
		let error_msg = match err {
			...
			CaughtError::Exception(ex) => {
				let error_name =
					get_class_name(&ex).unwrap_or(None).unwrap_or(String::from("Error"));
...
fn load<'js>(ctx: Ctx<'js>, filename: String, options: Opt<Object<'js>>) -> Result<Value<'js>> {
	let mut eval_options = EvalOptions { global: true, strict: false, backtrace_barrier: false };
